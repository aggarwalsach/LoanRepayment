...
public class LoanServiceImpl implements LoanService {
    ...
    private void validateLoan(
        ...
        if(request.getEmiSchedules().size()
                != request.getTenure()) {

            throw new RuntimeException(
                    "Tenure and EMI count mismatch");
        }

        BigDecimal totalPrincipal =
                request.getEmiSchedules()
                        .stream()
                        .map(EmiScheduleRequest::getPrincipalDue)
                        .reduce(BigDecimal.ZERO,
                                BigDecimal::add);

        if(totalPrincipal.compareTo(
                request.getLoanAmount()) != 0) {
        ...
        BigDecimal totalInterest =
                request.getEmiSchedules()
                        .stream()
                        .map(EmiScheduleRequest::getInterestDue)
                        .reduce(BigDecimal.ZERO,
                                BigDecimal::add);

        if(totalInterest.compareTo(
                request.getInterestAmount()) != 0) {
